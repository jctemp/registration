from reg.layers.spatial_transformer import SpatialTransformer

from reg.layers.swin_transformer import SwinTransformer
from reg.model.swin_transformer import SwinTransformer as MsSwinTransformer
...
class TransMorph(nn.Module):
    def __init__(self, config):
        ...
        self.transformer = MsSwinTransformer(
        ...
        self.up4 = DecoderBlock(
            ...
            skip_channels=(
                config.reg_head_chan if if_convskip else 0 if if_convskip else 0
            ),
            use_batchnorm=False,
        )  # 384, 160, 160, 256
        self.c1 = Conv3dReLU(config.in_chans, embed_dim // 2, 3, 1, use_batchnorm=False)
        self.c2 = Conv3dReLU(
            config.in_chans, config.reg_head_chan, 3, 1, use_batchnorm=False
        )
