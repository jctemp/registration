...
class SwinTransformerBlockMC(nn.Module):
    ...
    Args:
        ...
        mc_drop (float, optional): Block dropout rate.
    """
...
class BasicLayerMC(nn.Module):
    ...
    def forward(self, x, H, W, T):
        ...
        img_mask = torch.zeros((1, Hp, Wp, Tp, 1), device=x.device)  # 1 Hp Wp Tp 1
        h_slices = (
...
class SwinTransformerMC(nn.Module):
    r"""Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    Args:
        img_size (tuple(int)): Input image size. Default (224, 224, 224)
    ...
    def __init__(
        ...
        if self.ape:
            pretrain_img_size = to_3tuple(self.img_size)
    ...
    def train(self, mode=True):
        ...
        super(SwinTransformerMC, self).train(mode)
        self._freeze_stages()
