...
from .swin_transformer import window_partition, window_reverse, PatchMerging, PatchEmbed, SinPositionalEncoding3D
...
class WindowAttentionMC(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, rpe=True, drop=0.):
...
class SwinTransformerBlockMC(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=(7, 7, 7), shift_size=(0, 0, 0),
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, rpe=True, drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, mc_drop=0.15):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        ...
        self.attn = WindowAttentionMC(
            ...
            qkv_bias=qkv_bias, qk_scale=qk_scale, rpe=rpe, drop=mc_drop)

        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = MlpMC(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=mc_drop)
    ...
    def forward(self, x, mask_matrix):
        ...
        shifted_x = window_reverse(attn_windows, self.window_size, Hp, Wp, Tp)  # B H' W' T' C
...
class SwinTransformerMC(nn.Module):
    ...
    def __init__(
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayerMC(dim=int(embed_dim * 2 ** i_layer),
