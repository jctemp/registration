...
def load_losses(image_losses, flow_losses, delimiter=":"):
    image_losses = [str.split(s, delimiter) for s in str.split(str.lower(image_losses), "&")]
    flow_losses = [str.split(s, delimiter) for s in str.split(str.lower(flow_losses), "&")]
    return image_losses, flow_losses


def load_model_params(model_name, image_losses, flow_losses, optimizer_name, series_len):
    ...
    criterion_disp = None
    optimizer = CONFIGS_OPTIMIZER[optimizer_name] if optimizer_name is not None else None
