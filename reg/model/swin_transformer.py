...
def window_partition(x, window_size):
    ...
    x = x.view(B, H // window_size[0], window_size[0], W // window_size[1], window_size[1], L // window_size[2],
               window_size[2], C)
    windows = x.permute(0, 1, 3, 2, 4, 5, 6, 7).contiguous().view(-1, window_size[0], window_size[1], window_size[2], C)
    return windows
...
def window_reverse(windows, window_size, H, W, L):
    ...
    x = windows.view(B, H // window_size[0], W // window_size[1], L // window_size[2], window_size[0], window_size[1],
                     window_size[2], -1)
    x = x.permute(0, 1, 3, 2, 4, 5, 6, 7).contiguous().view(B, H, W, L, -1)
    return x
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        relative_coords = coords_flatten[:, :, None] - coords_flatten[:, None, :]  # 2, Wh*Ww, Wh*Ww Wt
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=(7, 7, 7), shift_size=(0, 0, 0),
        ...
        if min(self.input_resolution) <= self.window_size:
            ...
            m = min(self.input_resolution)
            self.window_size = (m, m, m)
        assert 0 <= min(self.shift_size) < min(
            self.window_size), f"shift_size must in 0-window_size, shift_size: {self.shift_size}, window_size: {self.window_size}"
        ...
        if self.shift_size > 0:
            ...
            H, W, L = self.input_resolution
            img_mask = torch.zeros((1, H, W, L, 1))  # 1 H W 1
            ...
            l_slices = (slice(0, -self.window_size[2]),
                        slice(-self.window_size[2], -self.shift_size[1]),
            ...
            # nW, window_size[0], window_size[1], window_size[2], 1
    ...
    def forward(self, x):
        ...
        x_windows = x_windows.view(-1, self.window_size[0] * self.window_size[1] * self.window_size[2],
                                   C)  # nW*B, window_size[0] * window_size[1] * window_size[2], C
        ...
        shifted_x = window_reverse(attn_windows, self.window_size, H, W, T)  # B H' W' C
    ...
    def flops(self):
        ...
        H, W, T = self.input_resolution
        # norm1
        flops += self.dim * H * W * T
        # W-MSA/SW-MSA
        nW = H * W * T / self.window_size[0] / self.window_size[1] / self.window_size[2]
        flops += nW * self.attn.flops(self.window_size[0] * self.window_size[1] * self.window_size[2])
        # mlp
        flops += 2 * H * W * T * self.dim * self.dim * self.mlp_ratio
        # norm2
        flops += self.dim * H * W * T
        return flops
...
class PatchMerging(nn.Module):
    ...
    Args:
        ...
        dim (int): Number of input channels.
        norm_layer (nn.Module, optional): Normalization layer.  Default: nn.LayerNorm
        reduce_factor (int): scale the output features of the linear layer
    """

    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm, reduce_factor=2):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=(224, 224, 1), patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        super().__init__()
        patch_size_t = (patch_size, patch_size, patch_size)
        patches_resolution = [img_size[0] // patch_size_t[0], img_size[1] // patch_size_t[1],
                              img_size[2] // patch_size_t[2]]
    ...
    def forward(self, x):
        ...
        assert H == self.img_size[0] and W == self.img_size[1] and T == self.img_size[1], \
    ...
    def flops(self):
        Ho, Wo, To = self.patches_resolution
        flops = Ho * Wo * To * self.embed_dim * self.in_chans * (
                self.patch_size[0] * self.patch_size[1] * self.patch_size[2])
        if self.norm is not None:
            flops += Ho * Wo * To * self.embed_dim
        return flops
...
class SwinTransformer(nn.Module):
    ...
    Args:
        img_size (tuple(int)): Input image size. Default (224, 224, 1)
    ...
    def __init__(self, img_size=(224, 224, 1), patch_size=4, in_chans=3,
