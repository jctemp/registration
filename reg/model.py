...
class TransMorphModule(pl.LightningModule):
    def __init__(
        self,
        net: TransMorph | TransMorphBspline | TransMorphBayes,
        criteria_image=None,
        criteria_flow=None,
        criterion_disp=None,
    ...
    def predict_segment(self, batch, fixed):
        ...
        assert (
            len(batch.shape) == len(fixed.shape)
            and batch.shape[:-1] == fixed.shape[:-1]
    ...
    def predict_series(self, batch, fixed):
        ...
        flows_out = torch.zeros(
            (batch.shape[0], 2, *(batch.shape[2:-1]), max_length), device=device
        )
        disp_out = (
            torch.zeros_like(warped_out)
            if is_diff
        ...
        for start_idx in range(0, max_length, max_segment_length):
            ...
            )  # need to offset by one due to fixed
            ...
            if is_diff:
                warped, flows, disp = self.net(batch_slice)
                disp_out[..., start_idx:end_idx] = disp[
                    ..., batch_slice_start:batch_slice_end
                ]
            else:
                warped, flows = self.net(batch_slice)

            warped_out[..., start_idx:end_idx] = warped[
                ..., batch_slice_start:batch_slice_end
            ]
            flows_out[..., start_idx:end_idx] = flows[
                ..., batch_slice_start:batch_slice_end
            ]
    ...
    def _get_preds_loss(self, batch, training=True):
        ...
        warped, flows, disp = (
            self.predict_segment(batch, fixed)
            if training
            else self.predict_series(batch, fixed)
        )
        ...
        for loss_fn, w in self.criteria_image:
            loss += (
                torch.mean(
                    torch.stack(
                        [loss_fn(warped[..., i], fixed) for i in range(series_len)]
                    )
                )
                * w
            )
        ...
        for loss_fn, w in self.criteria_flow:
            loss += (
                torch.mean(
                    torch.stack([loss_fn(flows[..., i]) for i in range(series_len)])
                )
                * w
            )
